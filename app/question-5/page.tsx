...
export default function Question5() {
  const [prepareWeather, setPrepareWeather] = useState("");
  const router = useRouter();
  const navigateToQuestion6 = () => {
    localStorage.setItem("q5", prepareWeather);
    router.push("/question-6");
  };

  useEffect(() => {
    let str = localStorage.getItem("q5");
    if (str) {
      setPrepareWeather(str);
    }
  }, []);
  ...
  return (
    <section className="flex min-h-[100vh] items-center">
      <div className="h-[640px] md:h-[700px]  w-full flex flex-col items-center gap-5">
        <p className="text-lg text-dark-blue">Question 5</p>
        <div className="flex flex-col items-center justify-center gap-4">
          <h1 className="text-3xl font-bold whitespace-pre-wrap text-center text-dark-blue">
            Have You Prepared{"\n"}Clothes for the Weather There?
          </h1>
          <div className="flex gap-3">
            <button
              onClick={() => setPrepareWeather("Yes")}
              className={
                "h-12 w-28 p-2 text-white font-bold rounded-lg hover:bg-vivid-blue " +
                (prepareWeather === "Yes" ? "bg-vivid-blue" : "bg-grey-grey ")
              }
            >
              Yes
            </button>
            <button
              onClick={() => setPrepareWeather("No")}
              className={
                "h-12 w-28 p-2 text-white font-bold rounded-lg hover:bg-vivid-blue " +
                (prepareWeather === "No" ? "bg-vivid-blue" : "bg-grey-grey ")
              }
            >
              No
            </button>
