...
export default function Question7() {
  const [prepareIternary, setPrepareIternary] = useState("");
  const router = useRouter();
  const navigateToEnd = () => {
    localStorage.setItem("q7", prepareIternary);
    router.push("/preview");
  };
  useEffect(() => {
    let str = localStorage.getItem("q7");
    if (str) {
      setPrepareIternary(str);
    }
  }, []);
  return (
    <section className="flex min-h-[100vh] items-center">
      <div className="h-[640px] md:h-[700px]  w-full flex flex-col items-center gap-5">
        <p className="text-lg text-dark-blue">Question 7</p>
        <div className="flex flex-col items-center justify-center gap-4">
          <h1 className="text-3xl font-bold whitespace-pre-wrap text-center text-dark-blue">
            Have You Prepared an Itinerary?
          </h1>
          <div className="flex gap-3">
            <button
              onClick={() => setPrepareIternary("Yes")}
              className={
                "h-12 w-28 p-2 text-white font-bold rounded-lg hover:bg-vivid-blue " +
                (prepareIternary === "Yes" ? "bg-vivid-blue" : "bg-grey-grey ")
              }
            >
              Yes
            </button>
            <button
              onClick={() => setPrepareIternary("No")}
              className={
                "h-12 w-28 p-2 text-white font-bold rounded-lg hover:bg-vivid-blue " +
                (prepareIternary === "No" ? "bg-vivid-blue" : "bg-grey-grey ")
              }
            >
              No
            </button>
          ...
          <p className="whitespace-pre-wrap text-center text-dark-blue">
            It's Optional, but You Will Need It{"\n"}for Certain Countries That
            Are Strict.
          </p>
        </div>
        {prepareIternary !== "" && (
