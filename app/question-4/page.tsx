...
export default function Question4() {
  const [buySouvenirs, setBuySouvenirs] = useState("");
  const router = useRouter();
  const navigateToQuestion5 = () => {
    localStorage.setItem("q4", buySouvenirs);
    router.push("/question-5");
  };
  useEffect(() => {
    let str = localStorage.getItem("q4");
    if (str) {
      setBuySouvenirs(str);
    }
  }, []);
  ...
  return (
    <section className="flex min-h-[100vh] items-center">
      <div className="h-[640px] md:h-[700px]  w-full flex flex-col items-center gap-5">
        <p className="text-lg text-dark-blue">Question 4</p>
        <div className="flex flex-col items-center justify-center gap-4">
          <h1 className="text-3xl font-bold whitespace-pre-wrap text-center text-dark-blue">
            Are You Planning To Buy{"\n"}Souvenirs or Local Food Items?
          </h1>
          <div className="flex gap-3">
            <button
              onClick={() => setBuySouvenirs("Yes")}
              className={
                "h-12 w-28 p-2 text-white font-bold rounded-lg hover:bg-vivid-blue " +
                (buySouvenirs === "Yes" ? "bg-vivid-blue" : "bg-grey-grey ")
              }
            >
              Yes
            </button>
            <button
              onClick={() => setBuySouvenirs("No")}
              className={
                "h-12 w-28 p-2 text-white font-bold rounded-lg hover:bg-vivid-blue " +
                (buySouvenirs === "No" ? "bg-vivid-blue" : "bg-grey-grey ")
              }
            >
              No
            </button>
